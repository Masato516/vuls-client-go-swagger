...
func (a *Client) HealthHealth(params *HealthHealthParams, opts ...ClientOption) (*HealthHealthOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		Params:             params,
		Reader:             &HealthHealthReader{formats: a.formats},
	...
	success, ok := result.(*HealthHealthOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for health#health: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
